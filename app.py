...
- Balance neto estimado: **{total_deficit - total_exceso:+.0f} horas**
    ...
    # =======================================================================
    # TAB 7 — INTERVENCIÓN POR ASIGNATURA T/P/SG
    # =======================================================================
    with tab7:
        st.subheader("Intervención por Asignatura — Análisis T/P/SG")

        horas_dict = data.get("horas_asignaturas", {})

        if not horas_dict:
            st.warning(
                "No se encontró la hoja **Horas_Asignaturas_SHOA** en el Excel. "
                "Verifica que el archivo tenga esa hoja y regenera el JSON."
            )
        else:
            # Computar análisis (cacheado en session_state)
            if "_df_exp" not in st.session_state or "_df_intv" not in st.session_state:
                df_exp, stats = build_topic_assignment_map(df_leaves, horas_dict)
                df_intv = compute_intervention_analysis(df_exp)
                st.session_state["_df_exp"]   = df_exp
                st.session_state["_df_intv"]  = df_intv
                st.session_state["_intv_stats"] = stats
            else:
                df_exp   = st.session_state["_df_exp"]
                df_intv  = st.session_state["_df_intv"]
                stats    = st.session_state["_intv_stats"]

            kpis_i = compute_intervention_kpis(df_intv)

            # ── Banner de validación ──────────────────────────────────────
            no_enc = stats.get("no_encontrados", set())
            st.info(
                f"📊 **{kpis_i['n_total']}** asignaturas analizadas  &nbsp;|&nbsp;  "
                f"⚠️ **{stats.get('divididas', 0)}** tópicos distribuidos por ';'  &nbsp;|&nbsp;  "
                + (f"⚠️ **{len(no_enc)}** códigos no encontrados en Horas_Asignaturas_SHOA: "
                   f"`{'`, `'.join(sorted(no_enc))}`"
                   if no_enc else "✅ Todos los códigos encontrados")
            )

            # ── KPIs ──────────────────────────────────────────────────────
            k1, k2, k3, k4 = st.columns(4)
            k1.metric("🔴 Asignaturas críticas",    kpis_i["n_critica"])
            k2.metric("🟠 Intervención alta",        kpis_i["n_alta"])
            k3.metric("🟢 Alineadas",                kpis_i["n_alin"])
            k4.metric("⏱️ Horas en exceso (total)", f"{kpis_i['exceso_total']:.1f} h")

            st.divider()

            # ── Gráfico 1: barras apiladas T/P/SG ─────────────────────────
            st.markdown("#### Ranking por Exceso — Horas Apiladas T / P / SG")

            df_plot1 = df_intv.sort_values("shoa_total", ascending=True)

            fig_i1 = go.Figure()
            for col, color, label in [
                ("shoa_T",  "#0077C8", "T — Teóricas"),
                ("shoa_P",  "#1A7A4A", "P — Prácticas"),
                ("shoa_SG", "#C8A84B", "SG — Auto estudio"),
            ]:
                fig_i1.add_trace(go.Bar(
                    name=label,
                    x=df_plot1[col],
                    y=df_plot1["codigo_asig"],
                    orientation="h",
                    marker_color=color,
                    hovertemplate=(
                        "<b>%{y}</b><br>" + label + ": %{x:.1f} h<extra></extra>"
                    ),
                ))

            # Línea del promedio internacional
            fig_i1.add_trace(go.Scatter(
                x=df_plot1["intl_avg"],
                y=df_plot1["codigo_asig"],
                mode="markers",
                name="Prom. Internacional",
                marker=dict(symbol="line-ew", color="#C0392B", size=14,
            ...
            # ── Gráfico 2: perfil detallado por asignatura ─────────────────
            st.markdown("#### Perfil Detallado por Asignatura")

            criticas_altas = df_intv[df_intv["clasificacion"].isin(["CRÍTICA", "ALTA"])]
            opciones_i = ["— Seleccionar —"] + criticas_altas["codigo_asig"].tolist()
            sel_i = st.selectbox(
                "Asignatura (CRÍTICA o ALTA):", opciones_i, key="sel_intv"
            )

            if sel_i != "— Seleccionar —":
                row_sel = df_intv[df_intv["codigo_asig"] == sel_i].iloc[0]
                topics_sel = df_exp[df_exp["codigo_asig"] == sel_i]

                c_dona, c_barra = st.columns(2)

                # Dona T/P/SG
                with c_dona:
                    fig_dona = go.Figure(go.Pie(
                        labels=["T — Teóricas", "P — Prácticas", "SG — Auto estudio"],
                        values=[row_sel["shoa_T"], row_sel["shoa_P"], row_sel["shoa_SG"]],
                        hole=0.45,
                        marker_colors=["#0077C8", "#1A7A4A", "#C8A84B"],
                    ))
                    fig_dona.update_layout(
                        title=f"{sel_i} — Distribución T/P/SG",
                        height=320, paper_bgcolor="#F2F4F7",
                ...
                # Barra SHOA vs Intl
                with c_barra:
                    fig_barra_i = go.Figure(go.Bar(
                        x=["SHOA", "Prom. Internacional"],
                        y=[row_sel["shoa_total"], row_sel["intl_avg"]],
                        marker_color=["#003366", "#C8A84B"],
                        text=[f"{row_sel['shoa_total']:.1f} h",
                              f"{row_sel['intl_avg']:.1f} h"],
                        textposition="outside",
                    ))
                    fig_barra_i.update_layout(
                        title=f"{sel_i} — Total vs Internacional",
                        yaxis_title="Horas",
                        height=320, plot_bgcolor="#FFFFFF",
                        paper_bgcolor="#F2F4F7",
                    )
                    st.plotly_chart(fig_barra_i, use_container_width=True)

                # Tabla tópicos IHO
                st.markdown(f"**Tópicos IHO cubiertos por {sel_i}** ({len(topics_sel)} tópicos)")
                df_top_show = topics_sel[[
                    "codigo_topico", "desc_topico",
                    "shoa_T", "shoa_P", "shoa_SG", "shoa_total",
                    "intl_avg_topico", "fue_dividida", "proporcion_pct"
                ]].copy()
                df_top_show.columns = [
                    "Código IHO", "Descripción",
                    "T", "P", "SG", "Total SHOA",
                    "Intl avg", "Dividida", "Proporción %"
                ]
                st.dataframe(
                    df_top_show.style.format({
                        "T": "{:.1f}", "P": "{:.1f}", "SG": "{:.1f}",
                        "Total SHOA": "{:.1f}", "Intl avg": "{:.1f}",
                        "Proporción %": "{:.1f}%",
                    }),
            ...
            # ── Gráfico 3: tres heatmaps T | P | SG ───────────────────────
            st.markdown("#### Mapa de Calor — Horas por Tipo (asignaturas críticas/altas)")

            df_ca = df_exp[df_exp["codigo_asig"].isin(
                criticas_altas["codigo_asig"].tolist()
            )].copy()

            if not df_ca.empty:
                for tipo, col_h, titulo in [
                    ("T",  "shoa_T",  "Horas Teóricas (T)"),
                    ("P",  "shoa_P",  "Horas Prácticas (P)"),
                    ("SG", "shoa_SG", "Auto Estudio (SG)"),
                ]:
                    piv = (df_ca.groupby(["codigo_asig", "codigo_topico"])[col_h]
                    ...
                    fig_hm = go.Figure(go.Heatmap(
                        ...
                        hovertemplate="Asignatura: %{y}<br>Tópico: %{x}<br>Horas: %{z:.1f}<extra></extra>",
                        colorbar=dict(title=tipo),
                    ))
                    fig_hm.update_layout(
                        title=titulo,
                        xaxis_title="Tópico IHO",
                        yaxis_title="Asignatura",
                        height=max(280, 28 * len(piv)),
                        plot_bgcolor="#FFFFFF",
                        paper_bgcolor="#F2F4F7",
            ...
            with col_fi1:
                fil_clf_i = st.multiselect(
                    "Clasificación",
                    ["CRÍTICA", "ALTA", "ALINEADA", "SUBESTIMADA"],
                    default=["CRÍTICA", "ALTA", "ALINEADA", "SUBESTIMADA"],
                    key="fil_clf_i",
                )
            with col_fi2:
                fil_perf = st.multiselect(
                    "Perfil dominante",
                    ["T", "P", "SG"],
                    default=["T", "P", "SG"],
                    key="fil_perf_i",
            ...
            with col_fi3:
                busq_i = st.text_input(
                    "Buscar asignatura", placeholder="Nombre o código…", key="busq_i"
                )

            df_tbl_i = df_intv[
                df_intv["clasificacion"].isin(fil_clf_i) &
                df_intv["perfil"].isin(fil_perf)
            ].copy()
            if busq_i:
                mask = (df_tbl_i["codigo_asig"].str.contains(busq_i, case=False, na=False) |
                        df_tbl_i["nombre_asig"].str.contains(busq_i, case=False, na=False))
                df_tbl_i = df_tbl_i[mask]

            def _color_clf_i(val):
                return {
                    "CRÍTICA":     "background-color:#FADBD8;color:#C0392B;font-weight:bold",
                    "ALTA":        "background-color:#FAE5D3;color:#E67E22;font-weight:bold",
                    "ALINEADA":    "background-color:#D5F5E3;color:#1A7A4A",
                    "SUBESTIMADA": "background-color:#D6EAF8;color:#2471A3",
                }.get(val, "")
            ...
            cols_show_i = [
                ...
                "shoa_T", "shoa_P", "shoa_SG", "shoa_total",
                "pct_T", "pct_P", "pct_SG", "perfil",
                "intl_avg", "delta_h", "delta_pct",
                "clasificacion", "n_topicos", "tipo_reducir",
            ]
            df_show_i = df_tbl_i[cols_show_i].copy()
            df_show_i.columns = [
                "Código", "Asignatura",
                "T", "P", "SG", "Total SHOA",
                "% T", "% P", "% SG", "Perfil",
                "Prom. Intl", "Δ (h)", "Δ (%)",
                "Clasificación", "N° Tópicos", "Tipo hora a reducir",
            ]

            try:
                styled_i = df_show_i.style.map(_color_clf_i, subset=["Clasificación"])
            except AttributeError:
                styled_i = df_show_i.style.applymap(_color_clf_i, subset=["Clasificación"])

            st.dataframe(
                styled_i.format({
                    "T": "{:.1f}", "P": "{:.1f}", "SG": "{:.1f}",
                    "Total SHOA": "{:.1f}", "Prom. Intl": "{:.1f}",
                    "% T": "{:.1f}%", "% P": "{:.1f}%", "% SG": "{:.1f}%",
                    "Δ (h)": "{:+.1f}", "Δ (%)": "{:+.1f}%",
                }),
                use_container_width=True,
            ...
            st.markdown(
                f"**{len(df_tbl_i)}** asignaturas · "
                f"Exceso total T: **{kpis_i['exceso_T']:.1f} h** | "
                f"P: **{kpis_i['exceso_P']:.1f} h** | "
                f"SG: **{kpis_i['exceso_SG']:.1f} h**"
            )
            ...
            # ── Exportación ───────────────────────────────────────────────
            st.markdown("#### Exportar")
            xlsx_intv = build_intervention_excel(df_intv, df_exp)
            st.download_button(
                label="📥 Exportar Intervención T/P/SG",
                data=xlsx_intv,
                file_name=f"SHOA_Intervencion_TPSG_{date.today().strftime('%Y%m%d')}.xlsx",
                mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                type="primary",
                use_container_width=True,
            )
